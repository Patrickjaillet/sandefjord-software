...
- [ ] Activer les GitHub Discussions sur le dépôt `sandefjord-software` et créer une catégorie dédiée (un fil de discussion par logiciel, mappé via l'`id` du logiciel plutôt que le `pathname` pour rester stable si les URLs changent) → **seule étape encore manuelle** : nécessite d'activer Discussions et d'installer l'app GitHub `giscus` depuis l'interface GitHub (consentement OAuth interactif, impossible à automatiser depuis cet environnement sans token) ; toute la partie code est prête et attend juste les deux identifiants générés par giscus.app — voir la section "Comments, likes, and sharing" du `README.md` pour la marche à suivre
